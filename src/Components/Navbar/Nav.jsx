import { ShoppingCart } from 'lucide-react';
import React from 'react';

const Nav = ({ cartCount }) => {
    return (
        <div className='px-5 sm:px-13 md:px-17 lg:px-20 py-4 flex justify-between items-center shadow-lg min-w-screen bg-white fixed z-20'>
            <div className="navbar-start flex  items-center gap-2 ">
                <div className="dropdown">
                    <div tabIndex={0} role="button" className="btn btn-ghost lg:hidden bg-purple-600 p-1">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="white"> 
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" stroke="white" /> 
                        </svg>
                    </div>
                    <ul tabIndex="-1" className="menu menu-sm dropdown-content bg-black rounded-box z-1 mt-3 w-52 p-2 shadow text-white">
                        <li><a>Features</a></li>
                        <li><a>Pricing</a></li>
                        <li><a>Testimonials</a></li>
                        <li><a>Products</a></li>
                        <li><a>FAQ</a></li>
                    </ul>
                </div>
                <p className='text-2xl sm:text-3xl md:text-4xl font-bold text-purple-500'>DigiTools</p>
            </div>
            
            <div className="navbar-center hidden lg:flex">
                <ul className="menu menu-horizontal px-1 text-gray-500 font-semibold text-sm">
                    <li><a>Features</a></li>
                    <li><a>Pricing</a></li>
                    <li><a>Testimonials</a></li>
                    <li><a>Products</a></li>
                    <li><a>FAQ</a></li>
                </ul>
            </div>
            
            <div className="navbar-end">
                <div className='flex gap-[5px] sm:gap-3 md:gap-5 lg:gap-8 justify-center items-center'>
                    <div className='relative flex items-center'>
                        <ShoppingCart color='black' className=' w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10' />
                        {cartCount > 0 && (
                            <span className='absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center font-bold'>
                                {cartCount}
                            </span>
                        )}
                    </div>

                    <p className='font-bold text-black text-sm sm:text-base md:text-lg lg:text-xl hover:underline cursor-pointer'>Login</p>
                    <button className=' btn bg-purple-600 text-white rounded-full font-bold cursor-pointer shadow text-sm sm:text-base md:text-lg lg:text-xl'>
                        Get Started
                    </button>
                </div>
            </div>
        </div>
    );
};

export default Nav;